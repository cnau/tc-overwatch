...
- **Stateless tokens — no cookies, no server-side sessions** on the main chain. The OAuth callback returns the JWT in the redirect URL fragment. Logout is a 204 no-op; the client discards the token. A server-side revocation list lands when there's a real reason.
...
- `/api/auth/*` is the API auth surface. `AuthController` owns `/me` and `/logout`.
