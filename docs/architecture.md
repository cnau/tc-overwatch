...
- **Stub login** (`POST /api/auth/dev-login`) is profile-gated to `local` — accepts any well-formed email, routes through the same `AuthService.signIn` the OAuth callback uses, including the invitation gate. Local dev exercises the gate the same way prod does.
...
- A separate Google Cloud OAuth client is used for local dev with `http://localhost:8080/login/oauth2/code/google` as the authorized redirect URI (Spring Security's default callback path; the backend is the redirect target, not the SPA — the SPA then receives the minted JWT via fragment). The OAuth client *secret* never lives in `application-local.yml` (which is checked in) — it lives in `.env.local` (gitignored) or in an OS keyring that the local profile picks up via env vars (`GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`).
