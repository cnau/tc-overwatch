...
- **Code style**: ktlint + detekt, both enforced in CI and both failing the build. ktlint owns formatting, detekt owns structure; overlapping rules are disabled on the detekt side in `detekt.yml` so each concern has one owner. detekt runs against a pinned Kotlin 2.0.21 analysis classpath — see § Scaffold notes.
...
- **CI lint steps**: ktlint + detekt (backend) and `npm run lint` (frontend) are enforced — scaffold-era `continue-on-error: true` flags removed after the first clean pass. detekt was task-disabled from the scaffold until #112 on the assumption it needed the unreleased 2.0.0; it actually needed two lines of config — its analysis classpath pinned to the Kotlin 2.0.21 it was compiled against, and `jvmTarget = "21"` because that embedded compiler rejects the toolchain's JDK 23. Both come out when 2.0.0 ships.
