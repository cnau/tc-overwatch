...
- **Vite proxy** in `vite.config.ts` forwards `/rpc/*`, `/oauth/*`, and `/api/*` to backend `localhost:8080`. The backend serves both gRPC-Web (RPC handlers) and HTTP (OAuth, auth, actuator) from the same port via spring-grpc-server-web's servlet bridge. Same-origin in the browser during dev — no CORS needed locally. Production uses cross-origin cookies under a shared parent domain; see `docs/architecture.md` § CORS.
