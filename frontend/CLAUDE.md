...
- **Stack pinned in `docs/architecture.md` § Frontend, beyond what's installed today**: the scaffold ships React + Vite + ESLint only. The full pinned stack is **Connect-ES, TanStack Query, Mantine v7+ (core + hooks; sub-packages per-feature), React Router v6+ with the Data Router, react-hook-form + Zod, Vitest + React Testing Library**. Install per-feature, not preemptively. Pin versions in `package.json` and update this file when you add one. See `docs/claude/react.md` for the patterns to follow once installed.
